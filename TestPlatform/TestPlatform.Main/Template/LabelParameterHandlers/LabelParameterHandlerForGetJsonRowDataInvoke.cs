...
namespace FW.TestPlatform.Main.Template.LabelParameterHandlers
{
    /// <summary>
    ///针对全局数据变量声明的标签参数处理
    ///格式:{$getjsonrowdatainvoke(data)}
    ///要求context中的Parameters中
    ///包含EngineType参数，参数类型为string
    [Injection(InterfaceType = typeof(LabelParameterHandlerForGetJsonRowDataInvoke), Scope = InjectionScope.Singleton)]
    public class LabelParameterHandlerForGetJsonRowDataInvoke : ILabelParameterHandler
    {
        private readonly ISelector<IFactory<IGenerateFuncInvokeService>> _generateFuncInvokeServiceSelector;
        private readonly ISelector<IFactory<IGetSeparatorService>> _getSeparatorServiceSelector;

        public LabelParameterHandlerForGetJsonRowDataInvoke(ISelector<IFactory<IGenerateFuncInvokeService>> generateFuncInvokeServiceSelector, ISelector<IFactory<IGetSeparatorService>> getSeparatorServiceSelector)
        {
        ...
        public async Task<string> Execute(TemplateContext context, string[] parameters)
        {
            ...
            if (parameters.Length != 1)
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { "{$getjsonrowdatainvoke(data)}", 1, parameters.Length }
                };
            ...
            string strTemp = await funService.Generate("GetJsonRowData", parameters);
