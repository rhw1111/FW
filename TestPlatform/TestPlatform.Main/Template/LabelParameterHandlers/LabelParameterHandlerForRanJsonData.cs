...
namespace FW.TestPlatform.Main.Template.LabelParameterHandlers
{
    /// <summary>
    ///针对全局数据变量声明的标签参数处理
    ///格式:{$ranjsondata(data,name)}
    ///要求context中的Parameters中
    ///包含EngineType参数，参数类型为string
    [Injection(InterfaceType = typeof(LabelParameterHandlerForRanJsonData), Scope = InjectionScope.Singleton)]
    public class LabelParameterHandlerForRanJsonData : ILabelParameterHandler
    {
        private readonly ISelector<IFactory<IGenerateFuncInvokeService>> _generateFuncInvokeServiceSelector;
        private readonly ISelector<IFactory<IGetSeparatorService>> _getSeparatorServiceSelector;

        public LabelParameterHandlerForRanJsonData(ISelector<IFactory<IGenerateFuncInvokeService>> generateFuncInvokeServiceSelector, ISelector<IFactory<IGetSeparatorService>> getSeparatorServiceSelector)
        {
        ...
        public async Task<string> Execute(TemplateContext context, string[] parameters)
        {
            ...
            if (parameters.Length != 2)
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { "{$ranjsondata(data,name)}", 2, parameters.Length }
                };
            ...
            string strTemp = await funService.Generate("RanJsonData", parameters);
