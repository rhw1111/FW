...
namespace FW.TestPlatform.Main.Template.LabelParameterHandlers
{
    ...
    public class LabelParameterHandlerForSendInit : ILabelParameterHandler
    {
        ...
        public LabelParameterHandlerForSendInit(ISelector<IFactory<IGenerateVarSettingService>> generateVarSettingServiceFactorySelector, ISelector<IFactory<IGetSeparatorService>> getSeparatorServiceSelector, ISelector<IFactory<IGetSpaceService>> getSpaceServiceSelector)
        {
        ...
        public async Task<string> Execute(TemplateContext context, string[] parameters)
        {
            ...
            if (parameters.Length < 1)
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { "{$sendinit(space)}", 1, parameters.Length }
                };
            ...
            if (!regex.IsMatch(parameters[0]))
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { "{$sendinit(space)}", "space", "Int" }
                };
