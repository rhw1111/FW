...
namespace FW.TestPlatform.Main.Template.LabelParameterHandlers
{
    ...
    public class LabelParameterHandlerForGetNameSerialNoInvoke : ILabelParameterHandler
    {
        ...
        public LabelParameterHandlerForGetNameSerialNoInvoke(ISelector<IFactory<IGenerateFuncInvokeService>> generateFuncInvokeServiceSelector, ISelector<IFactory<IGetSeparatorService>> getSeparatorServiceSelector)
        {
        ...
        public async Task<string> Execute(TemplateContext context, string[] parameters)
        {
            ...
            if (parameters.Length != 3)
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { "{$getnameserialnoinvoke(name,type,start)}", 3, parameters.Length }
                };
            ...
            string strTemp = await funService.Generate("GetNameSerialNo", parameters);
