...
namespace FW.TestPlatform.Main.Template.LabelParameterHandlers
{
    /// <summary>
    ///针对全局数据变量声明的标签参数处理
    ///格式:{$Sleep(number,direct,length)}
    ///要求context中的Parameters中
    ///包含EngineType参数，参数类型为string
    [Injection(InterfaceType = typeof(LabelParameterHandlerForSleep), Scope = InjectionScope.Singleton)]
    public class LabelParameterHandlerForSleep : ILabelParameterHandler
    {
        private readonly ISelector<IFactory<IGenerateVarInvokeService>> _generateVarInvokeServiceSelector;
        private readonly ISelector<IFactory<IGetSeparatorService>> _getSeparatorServiceSelector;

        public LabelParameterHandlerForSleep(ISelector<IFactory<IGenerateVarInvokeService>> generateVarInvokeServiceSelector, ISelector<IFactory<IGetSeparatorService>> getSeparatorServiceSelector)
        {
        ...
        public async Task<string> Execute(TemplateContext context, string[] parameters)
        {
            ...
            var funService = _generateVarInvokeServiceSelector.Choose($"{engineType}-{LabelParameterTypes.Sleep}").Create();
            string strTemp = await funService.Generate($"{LabelParameterTypes.Sleep}", parameters);
