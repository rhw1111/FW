...
namespace FW.TestPlatform.Main.Template.LabelParameterHandlers
{
    /// <summary>
    ///针对全局数据变量声明的标签参数处理
    ///格式:{$fireeventrequest(self,is_success,name,start_time,e)}
    ...
    public class LabelParameterHandlerForFireEventRequest : ILabelParameterHandler
    {
        ...
        public LabelParameterHandlerForFireEventRequest(ISelector<IFactory<IGenerateFuncInvokeService>> generateFuncInvokeServiceSelector, ISelector<IFactory<IGetSeparatorService>> getSeparatorServiceSelector)
        {
        ...
        public async Task<string> Execute(TemplateContext context, string[] parameters)
        {
            ...
            if (parameters.Length != 5)
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { "{$fireeventrequest(self,is_success,name,start_time,e)}", 5, parameters.Length }
                };
            ...
            string strTemp = await funService.Generate("FireEventRequest", parameters);
