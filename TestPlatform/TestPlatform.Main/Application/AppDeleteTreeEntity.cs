...
{
    [Injection(InterfaceType = typeof(IAppDeleteTreeEntity), Scope = InjectionScope.Singleton)]
    public class AppDeleteTreeEntity : IAppDeleteTreeEntity
    {
        private readonly ITreeEntityRepository _treeEntityRepository;

        public AppDeleteTreeEntity(ITreeEntityRepository treeEntityRepository)
        {
            _treeEntityRepository = treeEntityRepository;
        }

        public async Task Do(Guid treeEntityId, CancellationToken cancellationToken = default)
        {
            ...
            await treeEntity.Delete(cancellationToken);
        }
    }
}
