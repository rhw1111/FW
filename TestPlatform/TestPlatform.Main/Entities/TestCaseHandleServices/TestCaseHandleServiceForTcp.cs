    ...
    public class TestCaseHandleServiceForTcp : ITestCaseHandleService
    {
        ...
        public async Task Run(TestCase tCase, CancellationToken cancellationToken = default)
        {
            ...
            await ParallelHelper.ForEach(slaveHosts, 10,
                ...
                {
                    //先删除文件夹内现有的所有文件
                    await item.Host.SSHEndpoint.ExecuteCommand($"rm -rf {_testFilePath}{string.Format(_testFileName, "_*")}", cancellationToken);
                ...
                for (var index = 0;index <= item.Count - 1;index++)
                {
                    ...
                    slaveCommands.Add(
                        async (preResult) =>
                        {
                            ...
                            return await Task.FromResult($"locust -f {_testFilePath}{string.Format(_testFileName, $"_{innerIndex.ToString()}")} --worker --headless --master-host {tCase.MasterHost.Address} --master-port 5557 -t {configuration.Duration.ToString()} > {_testFilePath}{string.Format(_testLogFileName, $"_slave")} 2>&1 &");
                            //return await Task.FromResult($"locust -f {_testFilePath}{string.Format(_testFileName, $"_{innerIndex.ToString()}")} --logfile {_testFilePath}{string.Format(_testLogFileName, $"_slave")} --worker --headless --master-host 127.0.0.1 --master-port 5557 -t {configuration.Duration.ToString()} > {_testFilePath}{string.Format(_testOutFileName, $"_slave_")}{innerIndex.ToString()} 2>&1 &");
                            //return await Task.FromResult($"locust -f {_testFilePath}{string.Format(_testFileName, $"_{innerIndex.ToString()}")} --worker --headless --master-host 127.0.0.1 --master-port 5557 -t {configuration.Duration.ToString()} > {_testFilePath}{string.Format(_testLogFileName, $"_slave")} 2>&1 &");
                        }
                   );                  
                }
