        ...
        public async Task Run(TestCase tCase, CancellationToken cancellationToken = default)
        {
            ...
            List<Func<string?, Task<string>>> commands = new List<Func<string?, Task<string>>>()
            {
               ...
               async (preResult)=>
               {
                   ...
                   return await Task.FromResult($"locust -f {path}{string.Format(_testFileName,string.Empty)} --master --headless --expect-workers {slaveCount.ToString()} --master-bind-port {configuration.LocustMasterBindPort.ToString()} -t {configuration.Duration.ToString()} -u {configuration.UserCount.ToString()} -r {configuration.PerSecondUserCount.ToString()} > {path}{string.Format(_testLogFileName,string.Empty)} 2>&1 &");
               }
            };
                ...
                for (var index = 0; index <= item.Count - 1; index++)
                {
                    ...
                    slaveCommands.Add(
                        async (preResult) =>
                        {
                            ...
                            return await Task.FromResult($"locust -f {path}{string.Format(_testFileName, $"_{innerIndex.ToString()}")} --worker --headless --master-host {tCase.MasterHost.Address} --master-port {configuration.LocustMasterBindPort.ToString()} > {path}{string.Format(_testLogFileName, $"_slave_")}{innerIndex.ToString()} 2>&1 &");
