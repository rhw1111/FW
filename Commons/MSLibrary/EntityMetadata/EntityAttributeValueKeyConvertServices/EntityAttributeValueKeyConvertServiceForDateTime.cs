...
{
    /// <summary>
    /// 针对时间日期类型的实体属性值转换成关键字字符串服务
    /// </summary>
    [Injection(InterfaceType = typeof(EntityAttributeValueKeyConvertServiceForDateTime), Scope = InjectionScope.Singleton)]
    public class EntityAttributeValueKeyConvertServiceForDateTime : IEntityAttributeValueKeyConvertService
    {
        ...
        public async Task<object> ConvertFrom(EntityAttributeInfo attributeInfo, string strKey)
        {
            if (strKey == null)
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, attributeInfo.Type, $"{typeof(EntityAttributeValueKeyConvertServiceForDateTime).FullName},ConvertFrom" }
                };

                throw new UtilityException((int)Errors.EntityKeyValueRequireNotNull,fragment);
            }

            if (attributeInfo.Type != EntityAttributeTypes.DateTime)
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, attributeInfo.Type, typeof(EntityAttributeValueKeyConvertServiceForDateTime).FullName, typeof(DateTime).FullName, $"{typeof(EntityAttributeValueKeyConvertServiceForDateTime).FullName},ConvertFrom" }
                };
            ...
            if (!DateTime.TryParse(strKey, out DateTime result))
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, attributeInfo.Type, strKey, $"{typeof(EntityAttributeValueKeyConvertServiceForDateTime).FullName},ConvertFrom" }
                };
        ...
        public async Task<string> ConvertTo(EntityAttributeInfo attributeInfo, object value)
        {
            ...
            if (!(value is DateTime))
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, value.GetType().FullName, typeof(EntityAttributeValueKeyConvertServiceForDateTime).FullName, typeof(DateTime).FullName, $"{typeof(EntityAttributeValueKeyConvertServiceForDateTime).FullName},ConvertTo" }
                };
            ...
            if (attributeInfo.Type != EntityAttributeTypes.DateTime)
            {
                ...
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, attributeInfo.Type, value.GetType().FullName, $"{typeof(EntityAttributeValueKeyConvertServiceForDateTime).FullName},ConvertTo" }
                };

                throw new UtilityException((int)Errors.EntityAttributeMetadataValueTypeNotMatchActual, fragment);
            }

            return await Task.FromResult(((DateTime)value).ToString());
        }
    }
}
