...
{
    /// <summary>
    /// 针对可空高精度浮点型类型的实体属性值转换成关键字字符串服务
    /// </summary>
    [Injection(InterfaceType = typeof(EntityAttributeValueKeyConvertServiceForDecimalNullAble), Scope = InjectionScope.Singleton)]
    public class EntityAttributeValueKeyConvertServiceForDecimalNullAble : IEntityAttributeValueKeyConvertService
    {
        public async Task<object> ConvertFrom(EntityAttributeInfo attributeInfo, string strKey)
        {
            ...
            if (attributeInfo.Type != EntityAttributeTypes.DecimalNullAble)
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, attributeInfo.Type, typeof(EntityAttributeValueKeyConvertServiceForDecimalNullAble).FullName, typeof(decimal?).FullName, $"{typeof(EntityAttributeValueKeyConvertServiceForDecimalNullAble).FullName},ConvertFrom" }
                };
            ...
            if (!Decimal.TryParse(strKey, out decimal result))
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, attributeInfo.Type, strKey, $"{typeof(EntityAttributeValueKeyConvertServiceForDecimalNullAble).FullName},ConvertFrom" }
                };
        ...
        public async Task<string> ConvertTo(EntityAttributeInfo attributeInfo, object value)
        {
            ...
            if (!(value is decimal?))
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, value.GetType().FullName, typeof(EntityAttributeValueKeyConvertServiceForDecimalNullAble).FullName, typeof(decimal?).FullName, $"{typeof(EntityAttributeValueKeyConvertServiceForDecimalNullAble).FullName},ConvertTo" }
                };
            ...
            if (attributeInfo.Type != EntityAttributeTypes.DecimalNullAble)
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, attributeInfo.Type, value.GetType().FullName, $"{typeof(EntityAttributeValueKeyConvertServiceForDecimalNullAble).FullName},ConvertTo" }
                };
