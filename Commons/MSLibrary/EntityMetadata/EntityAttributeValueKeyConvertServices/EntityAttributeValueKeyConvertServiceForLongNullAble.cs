...
{
    /// <summary>
    /// 针对可空长整型类型的实体属性值转换成关键字字符串服务
    /// </summary>
    [Injection(InterfaceType = typeof(EntityAttributeValueKeyConvertServiceForLongNullAble), Scope = InjectionScope.Singleton)]
    public class EntityAttributeValueKeyConvertServiceForLongNullAble : IEntityAttributeValueKeyConvertService
    {
        ...
        public async Task<object> ConvertFrom(EntityAttributeInfo attributeInfo, string strKey)
        {
            ...
            if (attributeInfo.Type != EntityAttributeTypes.LongNullAble)
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, attributeInfo.Type, typeof(EntityAttributeValueKeyConvertServiceForLongNullAble).FullName, typeof(long?).FullName, $"{typeof(EntityAttributeValueKeyConvertServiceForLongNullAble).FullName},ConvertFrom" }
                };
            ...
            if (!Int64.TryParse(strKey, out long result))
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, attributeInfo.Type, strKey, $"{typeof(EntityAttributeValueKeyConvertServiceForLongNullAble).FullName},ConvertFrom" }
                };
        ...
        public async Task<string> ConvertTo(EntityAttributeInfo attributeInfo, object value)
        {
            ...
            if (!(value is long?))
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, value.GetType().FullName, typeof(EntityAttributeValueKeyConvertServiceForLongNullAble).FullName, typeof(long?).FullName, $"{typeof(EntityAttributeValueKeyConvertServiceForLongNullAble).FullName},ConvertTo" }
                };
            ...
            if (attributeInfo.Type != EntityAttributeTypes.LongNullAble)
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { attributeInfo.EntityInfo.EntityType, attributeInfo.Name, attributeInfo.Type, value.GetType().FullName, $"{typeof(EntityAttributeValueKeyConvertServiceForLongNullAble).FullName},ConvertTo" }
                };

                throw new UtilityException((int)Errors.EntityAttributeMetadataValueTypeNotMatchActual, fragment);
            }

            return await Task.FromResult(((long?)value).Value.ToString());
        }
    }
}
