...
{
    [Injection(InterfaceType = typeof(IAppGetKeepLoginUrlAndLogoutUrl), Scope = InjectionScope.Singleton)]
    public class AppGetKeepLoginUrlAndLogoutUrl : IAppGetKeepLoginUrlAndLogoutUrl
    {
        private ISecurityService _securityService;
        private ISystemLoginEndpointRepository _systemLoginEndpointRepository;

        public AppGetKeepLoginUrlAndLogoutUrl(ISecurityService securityService, ISystemLoginEndpointRepository systemLoginEndpointRepository)
        {
            _securityService = securityService;
            _systemLoginEndpointRepository = systemLoginEndpointRepository;
        }

        public async Task<AppGetKeepLoginUrlAndLogoutUrlResult> Do(string strToken)
        {
            ...
            if (!jwtResult.ValidateResult.Result)
            {
                var fragment = new TextFragment()
                {
                    ...
                    ReplaceParameters = new List<object>() { "MSLibrary.SystemToken.Application.AppGetKeepLoginUrlAndLogoutUrl", strToken, jwtResult.ValidateResult.Description }
                };
            ...
            AppGetKeepLoginUrlAndLogoutUrlResult result = new AppGetKeepLoginUrlAndLogoutUrlResult();

            //var keepLoginUrl=await systemLoginEndpoint.GetKeepLoginUrl(strToken);
            //var logoutUrl = await systemLoginEndpoint.GetLogoutUrl(strToken);

            //result.KeepLoginUrl = keepLoginUrl;
