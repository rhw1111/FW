...
namespace MSLibrary.Template.LabelParameterHandlers
{
    /// <summary>
    /// 当前UTC时间字符串
    /// 格式:{$currentutcdatetime(yyyyMMdd)}
    /// 参数为时间的格式化字符串
    /// </summary>
    [Injection(InterfaceType = typeof(LabelParameterHandlerForUTCDateTime), Scope = InjectionScope.Singleton)]
    public class LabelParameterHandlerForUTCDateTime : ILabelParameterHandler
    {
        public async Task<string> Execute(TemplateContext context, string[] parameters)
        {
            ...
            var strDate=DateTime.UtcNow.ToString(format);
