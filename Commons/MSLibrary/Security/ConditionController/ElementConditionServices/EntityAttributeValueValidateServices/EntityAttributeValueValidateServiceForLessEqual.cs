...
namespace MSLibrary.Security.ConditionController.ElementConditionServices.EntityAttributeValueValidateServices
{
    ...
    /// 针对小于等于运算符的实体属性值检查服务
    /// </summary>
    [Injection(InterfaceType = typeof(EntityAttributeValueValidateServiceForLessEqual), Scope = InjectionScope.Singleton)]
    public class EntityAttributeValueValidateServiceForLessEqual : IEntityAttributeValueValidateService
    {
        ...
        {
            string correctText = string.Format(StringLanguageTranslate.Translate(TextCodes.EntityAttributeValueValidateOperatorLessEqualCorrectText, "实体类型为{0}、实体关键字为{1}的实体记录的属性{2}的值小于等于{3}"), entityType, entityKey, attributeName, checkValue);
            string errorText = string.Format(StringLanguageTranslate.Translate(TextCodes.EntityAttributeValueValidateOperatorLessEqualErrorText, "实体类型为{0}、实体关键字为{1}的实体记录的属性{2}的值大于{3}"), entityType, entityKey, attributeName, checkValue);
