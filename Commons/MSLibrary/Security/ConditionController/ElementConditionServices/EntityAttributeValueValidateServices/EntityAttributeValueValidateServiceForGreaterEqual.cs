...
namespace MSLibrary.Security.ConditionController.ElementConditionServices.EntityAttributeValueValidateServices
{
    ...
    /// 针对大于等于运算符的实体属性值检查服务
    /// </summary>
    [Injection(InterfaceType = typeof(EntityAttributeValueValidateServiceForGreaterEqual), Scope = InjectionScope.Singleton)]
    public class EntityAttributeValueValidateServiceForGreaterEqual : IEntityAttributeValueValidateService
    {
        public async Task<EntityAttributeValueValidateServiceResult> Validate(string entityType, string entityKey, string attributeName, object attributeValue, string valueType, string checkValue)
        {
            ...
            string correctText = string.Format(StringLanguageTranslate.Translate(TextCodes.EntityAttributeValueValidateOperatorGreaterEqualCorrectText, "实体类型为{0}、实体关键字为{1}的实体记录的属性{2}的值大于等于{3}"), entityType, entityKey, attributeName, checkValue);
            string errorText = string.Format(StringLanguageTranslate.Translate(TextCodes.EntityAttributeValueValidateOperatorGreaterEqualErrorText, "实体类型为{0}、实体关键字为{1}的实体记录的属性{2}的值小于{3}"), entityType, entityKey, attributeName, checkValue);
