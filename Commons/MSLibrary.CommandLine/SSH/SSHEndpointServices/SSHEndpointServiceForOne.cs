...
namespace MSLibrary.CommandLine.SSH.SSHEndpointServices
{
    ...
    /// 
    /// </summary>
    [Injection(InterfaceType = typeof(SSHEndpointServiceForOne), Scope = InjectionScope.Singleton)]
    public class SSHEndpointServiceForOne : ISSHEndpointService
    {
        private static ConcurrentDictionary<string, Pool<object>> _sftpClientPools = new ConcurrentDictionary<string, Pool<object>>();
        private static ConcurrentDictionary<string, Pool<object>> _sshClientPools = new ConcurrentDictionary<string, Pool<object>>();

        private static int SftpClientPoolLength { get; set; } = 1;
        private static int SshClientPoolLength { get; set; } = 1;
        ...
        public async Task ExecuteCommand(string configuration, Func<ISSHEndpointCommandService, Task> action, int timeoutSeconds = -1, CancellationToken cancellationToken = default)
        {
            await exceptionHandle(async () =>
            {
                await sshClientExecute(configuration, async (client) =>
                {
                    SSHEndpointCommandServiceForOne service = new SSHEndpointCommandServiceForOne(client, timeoutSeconds);
        ...
        public async Task UploadFile(string configuration, Func<ISSHEndpointUploadFileService, Task> action, int timeoutSeconds = -1, CancellationToken cancellationToken = default)
        {
            await exceptionHandle(async () =>
            {
                await sftpClientExecute(configuration, async (client) =>
                {
                    ...
                    SSHEndpointUploadFileServiceForOne service = new SSHEndpointUploadFileServiceForOne(client);
        ...
        {
            if (!_sftpClientPools.TryGetValue(configuration, out Pool<object>? pool))
            {
                ...
                lock (_sftpClientPools)
                {
                    ...
                    {
                        pool = new Pool<object>($"SftpClient-{configuration}",
            null,
            null,
            null,
            null,
        ...
        {
            if (!_sshClientPools.TryGetValue(configuration, out Pool<object>? pool))
            {
                var configurationObj = getConfiguration(configuration);
                lock (_sshClientPools)
                {
                    ...
                    {
                        pool = new Pool<object>($"SshClient-{configuration}",
            null,
            null,
            null,
            null,
    ...
    public class SSHEndpointUploadFileServiceForOne : ISSHEndpointUploadFileService
    {
        private readonly SftpClient _sftpClient;

        public SSHEndpointUploadFileServiceForOne(SftpClient sftpClient)
        {
    ...
    public class SSHEndpointCommandServiceForOne : ISSHEndpointCommandService
    {
        private readonly SshClient _sshClient;
        private int _timeoutSeconds;
        public SSHEndpointCommandServiceForOne(SshClient sshClient, int timeoutSecond = -1)
        {
