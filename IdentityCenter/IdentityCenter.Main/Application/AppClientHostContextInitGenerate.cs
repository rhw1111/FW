...
{
    [Injection(InterfaceType = typeof(IAppClientHostContextInitGenerate), Scope = InjectionScope.Singleton)]
    public class AppClientHostContextInitGenerate : IAppClientHostContextInitGenerate
    {
        private readonly IIdentityClientHostRepositoryCacheProxy _identityClientHostRepositoryCacheProxy;
        ...
        public AppClientHostContextInitGenerate(IIdentityClientHostRepositoryCacheProxy identityClientHostRepositoryCacheProxy, ISystemConfigurationService systemConfigurationService,
        ...
        {
            _identityClientHostRepositoryCacheProxy = identityClientHostRepositoryCacheProxy;
        ...
        public async Task<IClientHostContextInit> Do()
        {
            var appName = await _systemConfigurationService.GetIdentityClientApplicationName();

            var host = await _identityClientHostRepositoryCacheProxy.QueryByName(appName);
            if (host == null)
            {
                var fragment = new TextFragment()
                {
                    Code = IdentityCenterTextCodes.NotFoundIdentityClientHostByName,
                    DefaultFormatting = "找不到名称为{0}的认证主机",
                    ReplaceParameters = new List<object>() { appName }
                };

                throw new UtilityException((int)IdentityCenterErrorCodes.NotFoundIdentityClientHostByName, fragment, 1, 0);
            }
            ...
            return new ClientHostContextInitDefault(claimContextGenerator, claims);
        }
    }

    public class ClientHostContextInitDefault : IClientHostContextInit
    {
        private readonly ClaimContextGenerator _claimContextGenerator;
        private readonly ClaimsIdentity _identity;

        public ClientHostContextInitDefault(ClaimContextGenerator claimContextGenerator, ClaimsIdentity identity)
        {
